...
export function ItemView({ data, entityId, onEdit, onSchedule, onOutcome, onArchive, onRestore }: {
  ...
  return <main><ViewShell eyebrow={type.singularName} title={entity.title} aside={entity.archivedAt ? 'Archived' : 'Active'}>
    ...
    <section className="item-section item-reference"><h3>Record</h3><p>{type.singularName} · <code>{entity.id}</code>{creation ? ` · ${creation.eventType === 'entity.migrated' ? 'migrated' : 'created'} ${new Date(creation.occurredAt).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium' })}` : ''}</p></section>
