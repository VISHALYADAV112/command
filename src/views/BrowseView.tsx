...
export function BrowseView({ data, typeKey, loadPage, onType, onOpenItem, onCapture, onOpenSettings }: {
  ...
  return <main><ViewShell eyebrow="Registry browse" title={type.pluralName} aside={remoteEntities ? `${visibleEntities.length}${hasNext ? '+' : ''} on page` : `${entities.length} records`}>
    ...
    {visibleEntities.length === 0 ? <EmptyState message={query || Object.values(filters).some(Boolean) ? 'No records match the current search and filters.' : `No ${type.pluralName.toLocaleLowerCase()} yet.`} /> : <div className="registry-list">{visibleEntities.map((entity) => <button type="button" className="registry-row" key={entity.id} onClick={() => onOpenItem(entity.id)}><strong>{entity.title}</strong>{visibleFields.map((field) => <span key={field.key}>{field.label}: {displayFieldValue(entity.fields[field.key])}</span>)}<small>{openCommitmentCount(data, entity.id)} open</small></button>)}</div>}
