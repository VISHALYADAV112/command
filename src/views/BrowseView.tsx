...
export function BrowseView({ data, typeKey, loadPage, onType, onOpenItem, onCapture, onOpenSettings }: {
  ...
  return <main><ViewShell eyebrow="Section 04 · Registry classifieds" title={type.pluralName} aside={<button className="capture-button" type="button" onClick={() => onCapture(type.typeKey)}>+ Capture</button>}>
    <div className="registry-controls">
      <input className="registry-search" type="search" aria-label="Search the registry" value={query} onChange={(event) => { setQuery(event.target.value); setPage(0) }} placeholder="Search the registry" />
      <select className="rule-select" aria-label="Record class" value={type.typeKey} onChange={(event) => { setFilters({}); setQuery(''); setPage(0); onType(event.target.value) }}>{data.entityTypes.filter((item) => item.isActive).map((item) => <option key={item.id} value={item.typeKey}>{item.pluralName}</option>)}</select>
      {filterable.map((field) => <select className="rule-select" aria-label={field.label} key={field.key} value={filters[field.key] ?? ''} onChange={(event) => { setFilters((current) => ({ ...current, [field.key]: event.target.value })); setPage(0) }}><option value="">All {field.label.toLocaleLowerCase()}</option>{field.options.map((option) => <option key={option} value={option}>{field.label}: {option}</option>)}</select>)}
      <span className="registry-count">{remoteEntities ? `${visibleEntities.length}${hasNext ? '+' : ''} on page` : `${entities.length} of ${data.entities.filter((item) => item.entityTypeId === type.id && !item.archivedAt).length}`}</span>
    </div>
