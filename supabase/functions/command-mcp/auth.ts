...
export function tokenVerifier(url: string, publishableKey: string): OAuthTokenVerifier {
  return {
    async verifyAccessToken(token: string): Promise<AuthInfo> {
      ...
      return tokenAuthInfo(token, data.user.id)
    },
  }
}

export function tokenAuthInfo(token: string, userId: string): AuthInfo {
  ...
  if (typeof claims.client_id !== 'string' || !claims.client_id.trim()) {
    throw new OAuthError(OAuthErrorCode.InvalidToken, 'Access token has no OAuth client.')
  }
  return {
    token,
