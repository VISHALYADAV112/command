-- Phase 6: proposals must still target active, reviewable records when applied.
...
begin
  if new.operation not in ('schedule', 'complete', 'cancel') then
    return new;
  end if;
...
revoke all on function public.enforce_agent_proposal_current_target()
  from public, anon, authenticated;

create trigger agent_proposals_enforce_current_target
  before insert or update on public.agent_proposals
  for each row execute function public.enforce_agent_proposal_current_target();
